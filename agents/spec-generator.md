...
- **Dependencies:** List packages by name and purpose only. Do NOT include version numbers or ranges. Version numbers in specs are stale by definition (the spec-generator's training data lags behind current releases). The implementation session MUST determine versions by using the framework's official scaffolding tool (e.g., `create-next-app@latest`) or by running `npm install <package>@latest` and letting npm resolve current stable versions. Add a note at the bottom of the Dependencies table: "Versions: Use `create-next-app@latest` (or equivalent scaffolding) to get current stable versions. Do not copy version numbers from this spec."
