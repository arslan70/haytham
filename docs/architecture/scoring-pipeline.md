...
| Score six dimensions | Rate the idea on the six dimensions above. Each score must be backed by specific evidence from an earlier stage, and the same evidence can't be reused for multiple scores. |
...
Three rules prevent low-quality scores:

1. **No parroting.** Evidence that repeats the scoring criteria word-for-word is rejected.
2. **High scores need a source.** Scores of 4+ must cite a specific earlier stage.
3. **No double-dipping.** Each dimension needs its own distinct evidence.
