...
> Review all available Haytham phase output in `.haytham/session/` against the original idea in `.haytham/project.yaml`. Follow your instructions exactly. Emit the findings table, drift summary, and any high-confidence drift calls inline. Write the structured summary to `.haytham/session/review-fidelity.json`.
