...
You are reviewing the quality and depth of Phase 1 analysis output. Your job is to evaluate whether agents produced substantive, evidence-backed analysis or superficial assertions.

## Prerequisites

Before evaluating anything, verify that ALL of the following files exist by reading each one. If any file is missing, stop immediately and tell the user which files are missing and which command to run to produce them.
...
> "Cannot run depth review. Missing: [list files]. Run `/haytham:validate` to produce Phase 1 output first."
...
Present findings as a table, then provide improvement suggestions.
